    ...
    <ion-content v-if="has_load">
      ...
      <div style="padding-top: 0.5rem">
        <ion-list v-if="params.momos.length">
          ...
          >
            <ion-radio
              ...
              slot="start" @click="set_momo(momo.id)"
            ></ion-radio>
            <ion-label @click="set_momo(momo.id)" >
              <h3>{{ momo.number }}</h3>
              <p>{{ momo.get_name }}</p>
            </ion-label>
            <ion-button
        ...
        <div style="padding: 1rem">
          <ion-button @click="mOpen = true" color="secondary" expand="full"
...
const get_params = async () => {
  ...
  const res = await axios.get("v2/api/get_sparams/", {
    headers: {
      Authorization: `Bearer ${await access_tok("/seller/params/", router, load)}`,
    },
  });
  if (res.data["done"]) {
...
const set_momo = async (id) => {
  ...
  form.append("id", `${id}`);
  const resp = await axios.post("v2/api/set_momo/", form, {
    headers: {
