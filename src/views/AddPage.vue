  ...
  <ion-page>
    <ion-header mode="ios">
      <ion-toolbar>
        ...
        <ion-title>Ajoutez une Catégorie</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content v-else>
      <div class="body_all">
        <div class="aid">
          ...
          <div>Choississez une catégorie à ajouter</div>
        </div>
            ...
            <div class="choose_cont" id="ssub" v-if="stp == 3">
              <ion-list style="background: rgb(238, 238, 238)">
                <div v-for="ss in get_cur_elts(stp)" :key="ss.id" class="d_sp">
                  <ion-item @click="(ssub = ss.id), stp++" button detail="true">
...
const add_sub = async () => {
  ...
  if (resp.data["done"]) {
    load.dismiss();
    router.push(`/seller/product/${resp.data["result"]}`);
  }
};
